...
describe('Your Test Suite Name', () => {
  it('Should perform actions on the website', () => {
    ...
    cy.get('#Farmlist > div').each(($farmElement, index) => {
      ...
      cy.get('#farm_details_section > div.col-md-7 > p:nth-child(2)', { withinSubject: $farmElement, timeout: 10000 })
        .invoke('text')
        .then((farmInfo) => {
          // Only display the current farm type
          cy.log(`Farm Type: ${farmInfo}`);

          switch (true) {
          case farmInfo.includes('Farm Type: farm'):
            // Scroll to the map element
            cy.wrap(null).then(far);
            // Perform other actions specific to "Farm" type
            break;
          case farmInfo.includes('Farm Type: tank'):
            // Scroll to the map element
            cy.wrap(null).then(tan);
            // Perform actions specific to "Tank" type
            break;
          case farmInfo.includes('Farm Type: polyhouse'):
            // Scroll to the map element
            cy.wrap(null).then(pol);
            // Perform actions specific to "Polyhouse" type
            break;
          case farmInfo.includes('Farm Type: garden'):
            // Scroll to the map element
            cy.wrap(null).then(gard);
            // Perform actions specific to "Garden" type
            break;
          }        
        });
    });
