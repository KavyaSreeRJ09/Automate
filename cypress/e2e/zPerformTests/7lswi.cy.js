...
describe('Login and Interact with Elements Test', () => {
  it('Should login and interact with elements', () => {
    ...
    // Scroll to the "LSWI" element
    cy.get('#lswiHead').scrollIntoView()
    
    // Click the "LSWI" button
    cy.get('#lswiBtn').click()
    
    // Wait for the "LSWI" data to load (adjust the wait time as needed)
    ...
    cy.clock().then((clock) => {
      ...
      cy.log(`Time taken for LSWI: ${duration} ms`)
    })
  })
})
describe('Login and Interact with Elements Test', () => {
