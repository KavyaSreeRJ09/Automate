...
describe('Login and Interact with Elements Test', () => {
  it('Should login and interact with elements', () => {
    ...
    // Scroll to the "NDVI" button element
    cy.get('#ndviBtn').scrollIntoView()
    
    // Click the "NDVI" button
    cy.get('#ndviBtn').click()
    
    // Wait for the "NDVI" data to load (adjust the wait time as needed)
    ...
    cy.clock().then((clock) => {
      ...
      cy.log(`Time taken for NDVI: ${duration} ms`)
    })
  })
})
